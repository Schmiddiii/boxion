...
pub struct ListRectColored<'a, T> {
    ...
    selected: u16,
    theme: &'a dyn Fn(T, bool)->(&'static dyn termion::color::Color, &'static dyn termion::color::Color),
}
...
impl<'a, T: Clone> ListRectColored<'a, T> {
    ...
    pub fn set_theme(&mut self, theme: &'a dyn Fn(T, bool)->(&'static dyn termion::color::Color, &'static dyn termion::color::Color)) {
        self.theme = theme;
    }
    ...
    pub fn show(&self, stdout: &mut dyn Write) {
        ...
        for (index, element) in self.elements.iter().enumerate() {
            let (fg_color, bg_color) = (self.theme)(element.0.clone(), index as u16 == self.selected);
            self.rect.write_colored_trimmed(stdout, &element.1, 0, index as u16, &*fg_color, &*bg_color).unwrap();
        }
    }
...
impl crate::rect::Rect {
    pub fn into_list_colored<'a,T>(self, theme: &'a dyn Fn(T, bool) -> (&'static dyn termion::color::Color, &'static dyn termion::color::Color)) -> Box<ListRectColored<'a, T>> {
